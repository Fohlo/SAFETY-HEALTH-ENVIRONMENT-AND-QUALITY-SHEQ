...
namespace PROPLASTICSAPI.Controllers
{
    ...
    [ApiController]
    public class PromouldingsProductionQualityControlController : Controller
    {
        [HttpGet]
        public ErrorMDL Get(string session, int Doc)
        {
            try
            {
                ...
                return new ErrorMDL { Success = JsonConvert.SerializeObject(PromouldingsProductionQualityControlBLL.get(session, Doc)) };
            }
            catch (Exception ex)
            {
                return new ErrorMDL
        ...
        public ErrorMDL Get(string session)
        {
            try
            {
                ...
                return new ErrorMDL { Success = JsonConvert.SerializeObject(PromouldingsProductionQualityControlBLL.get(session, query)) };
            }
            catch (Exception ex)
            {
                return new ErrorMDL
        ...
        [HttpPost]
        public ErrorMDL Post(string session, PostPROPQCMDL data)
        {
            try
            {
                SessionBLL.ValidateAppSession(session);
                return new ErrorMDL { Success = JsonConvert.SerializeObject(PromouldingsProductionQualityControlBLL.Post(session, data)) };
            }
            catch (Exception ex)
            {
                return new ErrorMDL
        ...
        [HttpPost]
        [Route("Update")]
        public ErrorMDL Post(string session, int Doc, PatchPROPQCMDL data)
        {
            try
            {
                SessionBLL.ValidateAppSession(session);
                return new ErrorMDL { Success = JsonConvert.SerializeObject(PromouldingsProductionQualityControlBLL.Patch(session, data, Doc)) };
            }
            catch (Exception ex)
            {
                return new ErrorMDL
