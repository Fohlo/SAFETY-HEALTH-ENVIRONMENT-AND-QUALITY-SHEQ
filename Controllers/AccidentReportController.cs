    ...
    public class AccidentReportController : Controller
    {
        ...
        public ErrorMDL Get(string session, int Doc)
        {
            try
            {
                ...
                return new ErrorMDL { Success = JsonConvert.SerializeObject(AccidentReportBLL.get(session, Doc)) };
            }
            catch (Exception ex)
            {
                return new ErrorMDL
        ...
        public ErrorMDL Get(string session)
        {
            try
            {
                ...
                return new ErrorMDL { Success = JsonConvert.SerializeObject(AccidentReportBLL.get(session, query)) };
            }
            catch (Exception ex)
            {
                return new ErrorMDL
        ...
        [HttpPost]
        public ErrorMDL Post(string session, PostAccidentReportMDL data)
        {
            try
            {
                SessionBLL.ValidateAppSession(session);
                return new ErrorMDL { Success = JsonConvert.SerializeObject(AccidentReportBLL.Post(session, data)) };
            }
            catch (Exception ex)
            {
                return new ErrorMDL
        ...
        [HttpPost]
        [Route("Update")]
        public ErrorMDL Post(string session, int Doc, PatchAccidentReportMDL data)
        {
            try
            {
                SessionBLL.ValidateAppSession(session);
        ...
        [HttpGet]
        [Route("GetByNatureOfIncident")]
        public ErrorMDL NatureOfInd(string session, string NatureOfInd)
        {
            try
            {
                SessionBLL.ValidateAppSession(session);
                var query = "ACCIDENTRPT?$filter = U_NatureOfInd eq '" + NatureOfInd + "'";
                return new ErrorMDL { Success = JsonConvert.SerializeObject(AccidentReportBLL.get(session, query)) };
            }
            catch (Exception ex)
            {
        ...
        public ErrorMDL Department(string session, string Department)
        {
            ...
            {
                SessionBLL.ValidateAppSession(session);
                var query = "ACCIDENTRPT?$filter = U_Department eq '" + Department + "'";
                return new ErrorMDL { Success = JsonConvert.SerializeObject(AccidentReportBLL.get(session, query)) };
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        [Route("GetByIncidentSeverity")]
        public ErrorMDL IncidentSeverity(string session, string IncidentSeverity)
        {
            try
            {
                SessionBLL.ValidateAppSession(session);
                var query = "ACCIDENTRPT?$filter = U_IncidentSeverity eq '" + IncidentSeverity + "'";
                return new ErrorMDL { Success = JsonConvert.SerializeObject(AccidentReportBLL.get(session, query)) };
            }
            catch (Exception ex)
            {
