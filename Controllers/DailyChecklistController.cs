    ...
    public class DailyChecklistController : ControllerBase
    {
        ...
        public ErrorMDL Get(string session, int Doc)
        {
            try
            {
                ...
                return new ErrorMDL { Success = JsonConvert.SerializeObject(DailyChecklistBLL.get(session, Doc)) };
            }
            catch (Exception ex)
            {
                return new ErrorMDL
        ...
        public ErrorMDL Get(string session)
        {
            try
            {
                ...
                return new ErrorMDL { Success = JsonConvert.SerializeObject(DailyChecklistBLL.get(session, query)) };
            }
            catch (Exception ex)
            {
                return new ErrorMDL
        ...
        [HttpPost]
        public ErrorMDL Post(string session, PostDailyChecklistMDL data)
        {
            try
            {
                SessionBLL.ValidateAppSession(session);
                return new ErrorMDL { Success = JsonConvert.SerializeObject(DailyChecklistBLL.Post(session, data)) };
            }
            catch (Exception ex)
            {
                return new ErrorMDL
        ...
        [HttpPost]
        [Route("Update")]
        public ErrorMDL Post(string session, int Doc, PatchDailyChecklistMDL data)
        {
            try
            {
                SessionBLL.ValidateAppSession(session);
                return new ErrorMDL { Success = JsonConvert.SerializeObject(DailyChecklistBLL.Patch(session, data, Doc)) };
            }
            catch (Exception ex)
            {
                return new ErrorMDL
